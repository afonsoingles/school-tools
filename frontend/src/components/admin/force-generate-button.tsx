...
export function ForceGenerateButton() {
  ...
  async function handleClick() {
    ...
    try {
      const res = await fetch("/api/v1/admin/force_generate_pending_feeds", {
        method: "POST",
      })
      const body = await res.json()

      if (res.ok && body.success) {
        setMessage(body.message ?? "Feed generation triggered.")
      } else {
        setError(body.message ?? "Failed to trigger feed generation.")
      }
    } catch {
