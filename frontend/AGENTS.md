...
- **Calendar** (`calendar-week-view.tsx`): **desktop (`md+`) is the ORIGINAL single-scroller design** — a static (non-scrolling) day-header row and one `scrollDesktopRef` vertical scroller with the hour gutter in-flow next to the columns. The scroller carries `no-scrollbar`, so desktop shows **zero visible scrollbars** (it still scrolls with wheel/trackpad). Do not give the desktop calendar panes, extra scroll containers, or a visible scrollbar. **Mobile (`max-md:`) is a four-pane frozen grid** — corner + day-header (`headerScrollRef`), hour gutter (`gutterScrollRef`, vertical only), and the main grid (`scrollMobileRef`, both axes) — with **programmatic scroll sync** (`syncGridScroll`/`syncHeaderScroll`/`syncGutterScroll` copying `scrollLeft`/`scrollTop`, re-entrancy-guarded via `syncRef` + equality checks). Both layouts share `renderHourLabels()`/`renderDayCells()`. Do NOT reintroduce `position: sticky left` — iOS Safari doesn't freeze it during two-axis touch panning (that was the bug). Day cells are `w-32 ... md:w-auto md:flex-1 md:min-w-0` so only mobile scrolls horizontally. Click→time math must stay `e.clientY - rect.top` (rect already includes scroll) — do not add `scrollTop` back. The initial scroll-to-first-class effect runs **once per week** (guarded by `scrolledWeekRef`) and targets both the desktop and mobile scrollers. Header/gutter panes carry `no-scrollbar` (a custom `@utility` in `globals.css`).
